...
# Create time-series aggregation with weapon types and ensure sorted time periods
time_weapon_data = (
...
# Sort first by time_period to ensure chronological order, then by weapon
...
# Filter significant weapon types but keep more of them
weapon_totals = time_weapon_data.groupby('weapon')['id'].sum()
major_weapons = weapon_totals[weapon_totals >= min_incidents].index
time_weapon_data = time_weapon_data[time_weapon_data['weapon'].isin(major_weapons)]

# Add cumulative totals for sizing reference
time_weapon_data['total_casualties'] = (
    time_weapon_data['fatalities'] + time_weapon_data['injuries']
)
